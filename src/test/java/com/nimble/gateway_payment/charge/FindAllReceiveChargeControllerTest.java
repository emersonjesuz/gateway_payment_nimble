...
public class FindAllReceiveChargeControllerTest {
    ...
    public void shouldReturn400IfStatusInvalid() throws Exception {
        this.mvc.perform(get("/charge/receive?status=INVALID")
    ...
    public void shouldReturn200AndListCharge() throws Exception {
        this.mvc.perform(get("/charge/receive")
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("Authorization", TestUtils.generatedToken(user2))
                )
    ...
    public void shouldReturn200AndListChargeWithFilterStatusPAID() throws Exception {
        this.mvc.perform(get("/charge/receive?status=PAID")
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("Authorization", TestUtils.generatedToken(user2))
                )
                .andExpect(MockMvcResultMatchers.status().isOk())
